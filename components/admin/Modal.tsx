...
import { setOptions, setIsShowModal } from '@/redux/slices/adminSlice'
...
const Modal = ({ handleChange, values, setFieldValue }: IProps) => {
  const { isShowModal } = useAppSelector(state => state.admin)
  const dispatch = useAppDispatch()

  const handleSubmit = async (values: IFormInput) => {
    try {
      const { optionObj: { optionType, itemPrice, itemName } } = values
      const parseOption = await JSON.parse(optionType)
      const data = { optionType: parseOption, itemName, itemPrice }
